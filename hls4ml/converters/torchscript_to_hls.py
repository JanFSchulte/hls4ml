...
class TorchScriptModelReader(object):
    """
    torchscript data reader to be used for extracting relevant information during conversion.
    """
    def __init__(self, config):
        self.torchscript_model = config['TorchScriptModel']
        self.state_dict = self.torchscript_model.state_dict()
    ...
    def get_weights_data(self, layer_name, var_name):
        ...
        between Keras and torchscript terminology.
        ...
        #Parameter mapping from torchscript to keras
        torch_paramap = {
        #Conv
        'kernel': 'weight', 
        #Batchnorm
        ...
        if var_name not in list(torch_paramap.keys()) + ['weight', 'bias']:
            raise Exception('torchscript parameter not yet supported!')
...
class TorchScriptFileReader(TorchScriptModelReader): #Inherit get_weights_data method
    def __init__(self, config):
        self.config = config

        if not torch.cuda.is_available():
            self.torchscript_model = torch.load(config['TorchScriptModel'], map_location=lambda storage, loc: storage)
        else:
            self.torchscript_model = torch.load(config['TorchScriptModel'])
...
def register_torchscript_layer_handler(layer_name, handler_func):
...
def get_supported_torchscript_layers():
    return list(layer_handlers.keys())

def torchscript_handler(*args):
...
def torchscript_to_hls(config):
    """ Convert TorchScript model to hls model from configuration.
    
    Parameters
    ----------
    config: dict
        torchscript configuration from yaml file or passed through API.
    ...
    Notes
    -----
    Only sequential torchscript models are supported for now.
    """
    ...
    reader = TorchScriptFileReader(config) if isinstance(config['TorchScriptModel'],str) else TorchScriptModelReader(config)
    input_shapes = [list(reader.input_shape)]
        
    model = reader.torchscript_model

    #Define layers to skip for conversion to HLS
    skip_layers = ['Dropout', 'Flatten', 'Sequential']
    
    #All supported layers
    supported_layers = get_supported_torchscript_layers() + skip_layers
    ...
    for layer_name, torchscript_layer in model.named_modules():
        #The 'orignal_name' attribute contains the original torchscript class name 
        torchscript_class = torchscript_layer.original_name
        
        #First module is the whole model's class
        if torchscript_class == model.original_name:
            continue
        
        if torchscript_class not in supported_layers:
            raise Exception('Unsupported layer {}'.format(torchscript_class))
        ...
        if torchscript_class in skip_layers:
            if torchscript_class == 'Sequential': #Ignore the mother module's class name
                continue

            if torchscript_class == 'Flatten':
        ...
        if torchscript_class in supported_layers:
            layer_counter += 1
        
        #Process the layer
        layer, output_shape = layer_handlers[torchscript_class](torchscript_layer, layer_name, input_shapes, reader, config)
